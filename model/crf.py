...
def crf_log_likelihood(args):
    inputs = args[0]
    tag_indices = args[1]
    transition_params=args[2]
    sequence_lengths=None
    num_tags = inputs.shape[2]
    data_len = inputs.shape[1]
    batch_size = inputs.shape[0]
    # cast type to handle different types
    tag_indices = tf.cast(tag_indices, dtype=tf.int32)

    if sequence_lengths is None:
        temp = tf.math.sign(tf.math.add(tf.math.abs(tf.math.sign(inputs)),1))
        sequence_lengths = tf.reduce_sum(tf.math.sign(tf.math.reduce_sum(temp,-1)),-1)
    if transition_params is None:
        initializer = tf.keras.initializers.GlorotUniform()
        transition_params = tf.Variable(
        lambda: initializer([num_tags, num_tags]), name="transitions")
...
def viterbi_decode(score,
                   transition_params):
    trellis = K.zeros_like(score)
    backpointers = K.zeros_like(score, dtype=tf.int32)
    trellis[0] = score[0]

    for t in range(1, score.shape[0]):
        v = K.expand_dims(trellis[t - 1], 1) + transition_params
        trellis[t] = score[t] + K.max(v, 0)
        backpointers[t] = K.argmax(v, 0)

    viterbi = [K.argmax(trellis[-1])]
    for bp in reversed(backpointers[1:]):
        viterbi.append(bp[viterbi[-1]])
    viterbi.reverse()

    viterbi_score = K.max(trellis[-1])
...
def crf_decode_backward(inputs,
                        state):
...
def crf_decode(args):
    ...
    """
    potentials,transition_params = args[0], args[1]
    sequence_length=None
    if sequence_length == None:
        temp = tf.math.sign(tf.math.add(tf.math.abs(tf.math.sign(potentials)),1))
        sequence_length = tf.reduce_sum(tf.math.sign(tf.math.reduce_sum(temp,-1)),-1)
